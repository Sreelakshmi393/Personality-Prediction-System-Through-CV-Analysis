...
def jobposts(request):
    if request.method == 'POST':
        ...
        print(name,email,mob,skills,experience,designation,degree,total_exp,companies,data_json)
        user=upload(userid=request.session['uid'],job_id=job_id,name=name,email=email,phone=mob,skill=skills,experience=experience,jsondata=data_json,image=filename)
        user.save()
